...
class Board {
    ...
    boolean legalMove(Move move) {
        if (move == null || numJumps() == JUMP_LIMIT) {
            return false;
        } else if (!canMove(whoseMove())) {
            return move.isPass() && numPieces(whoseMove()) > 1;
        } else {
            return get(move.fromIndex()).equals(whoseMove())
                    && get(move.toIndex()).equals(EMPTY);
        }
    }
    ...
    boolean canMove(PieceColor who) {
        for (int idx = 0; idx < _board.length; idx++) {
            if (_board[idx].equals(who)) {
                for (int i = -2; i < 3; i++) {
                    for (int j = -2; j < 3; j++) {
                        if (_board[idx + EXTENDED_SIDE * i + j].equals(EMPTY)) {
                            return true;
                        }
                    }
                }
    ...
    void makeMove(Move move) {
        ...
        if (move.isPass()) {
            ...
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    int tempIdx = neighbor(move.toIndex(), i, j);
                    if (_board[tempIdx].equals(whoseMove().opposite())) {
                        addUndo(tempIdx);
                        _board[tempIdx] = whoseMove();
            ...
            _board[move.toIndex()] = whoseMove(); _numJumps++;
        }
        _whoseMove = opponent;
        if (numJumps() == JUMP_LIMIT
                || !canMove(whoseMove()) && !canMove(whoseMove().opposite())) {
    ...
    void undo() {
        while (!_undoPieces.isEmpty()) {
            ...
            int idx = _undoSquares.pop();
            incrPieces(get(idx), -1);
            _board[idx] = p;
        }
    ...
    boolean legalBlock(char c, char r) {
        if (get(c, r).isPiece() || get(c, r).equals(BLOCKED)
                || !_allMoves.isEmpty()
                || c == 'a' && r == '1'
                || c == 'a' && r == '1' + SIDE - 1
                || c == 'a' + SIDE - 1 && r == '1'
                || c == 'a' + SIDE - 1 && r == '1' + SIDE - 1) {
            return false;
        }
        return true;
    }
    ...
    void setBlock(char c, char r) {
        ...
        char[] cols = {c, (char) ('h' - c + 'a' - 1)};
        char[] rows = {r, (char) ('8' - r + '1' - 1)};

        for (char row : rows) {
            for (char col : cols) {
                if (get(col, row).equals(BLOCKED)) {
                    continue;
                }
